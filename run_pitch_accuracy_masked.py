...
HMM emissions use masked cosine correlation (not raw MSE): per state, L2-normalize
pseudo-amplitude (10^(dB/20)) and template on the template-support mask, then take
their dot product (scale * cosine). Plain masked MSE favored wrong templates whose
masks sat on quiet bins (near-zero error); cosine matches spectral shape like the
reference cosine runner.
...
def note_template_linear(midi, fmin, n_bins, bins_per_octave, window_bins=2):
    """Same geometry as note_template_cqt but unnormalized (for masked MSE)."""
    tpl = np.zeros(n_bins, float)
    f0 = midi_to_hz(midi)
    harm_w = {1: 1.0, 2: 0.5, 3: 0.25}
    for h, w in harm_w.items():
...
def build_ASR_templates_linear_masked(
    states, fmin, n_bins, bins_per_octave, window_bins=2, mask_rel_thresh=0.08
):
    """
    Linear (unnormalized) templates + boolean masks for masked cosine emissions.
    Mask = bins where template >= mask_rel_thresh * max(template).
    Release / flat templates use all bins in the mask.
    """
    rows = []
    masks = []
    for st in states:
        ...
        tmax = float(np.max(t)) + 1e-12
        m = t >= (mask_rel_thresh * tmax)
        if not np.any(m):
            m = np.ones(n_bins, dtype=bool)

        rows.append(t)
        masks.append(m)

    return np.stack(rows, axis=0), np.stack(masks, axis=0)
...
def emissions_masked_mse(C_db, tpl_linear, tpl_mask, scale=1.0):
    """
    Masked cosine similarity in pseudo-amplitude space (same sign convention as
    emissions_cosine: higher is better).

    Per state s, let m be the boolean mask, x = C_lin * m, t = tpl * m; emission
    is scale * dot(x/||x||, t/||t||). Optimal-scaling MSE was near zero when m fell
    on bins with little energy (wrong pitch / misalignment), which dominated
    Viterbi incorrectly.
    """
    C_lin = np.power(10.0, C_db / 20.0)
    C_lin = np.maximum(C_lin, 1e-12)
    n_bins, T = C_lin.shape
    S = tpl_linear.shape[0]
    m_f = tpl_mask.astype(np.float64)

    B = np.zeros((S, T), dtype=np.float64)
    for s in range(S):
        m = m_f[s][:, np.newaxis]
        x = C_lin * m
        tv = tpl_linear[s] * m.ravel()
...
def emissions_ASR(
    ...
    B_base, scores = emissions_masked_mse(C_db, tpl_linear, tpl_mask, scale=scale_pitch)
    S, T = B_base.shape
...
def run(mp3_path: str, model_name: str, plot: bool = False):
    ...
    tpl_linear, tpl_mask = build_ASR_templates_linear_masked(
        states,
        fmin=FMIN,
        n_bins=N_BINS,
        bins_per_octave=BINS_PER_OCTAVE,
        window_bins=2,
        mask_rel_thresh=0.08,
    )
    B, _ = emissions_ASR(
        C_db,
        tpl_linear,
