...
HMM emissions use template-weighted log-likelihood (related to KL(q||p), not
KL(p||q)): frame pseudo-amplitude is normalized to p over bins; template is q.
Score = scale * sum_b q_b log p_b. Plain KL(p||q) made uniform release states
beat real notes on broad piano spectra; weighting by q fixes that.
...
Usage:
  python run_pitch_accuracy_kl.py --mp3 <path_to.mp3> --model ode_to_joy [--plot]
"""
...
def build_ASR_template_prob_distributions(
    states, fmin, n_bins, bins_per_octave, window_bins=2, prob_floor=1e-8
):
    """
    Per HMM state, a distribution q over CQT bins for KL(frame || q).
    Built from unnormalized linear partial template + floor, then normalized.
    Release / START-adjacent silence uses near-uniform distribution.
    """
    rows = []
    n_bins_i = int(n_bins)
    uniform = np.ones(n_bins_i, dtype=np.float64) / float(n_bins_i)
    for st in states:
        ph = st["phase"]
        if ph in ("A", "S"):
            raw = chord_template_linear(
                st["pitches"], fmin, n_bins_i, bins_per_octave, window_bins=window_bins
            ).astype(np.float64)
...
def emissions_kl(C_db, template_probs, scale=1.0, prob_floor=1e-10):
    """
    Emission = scale * sum_b q_sb log(p_b + eps) with p the per-frame normalized
    pseudo-amplitude and q the template row. For fixed frame, this matches the
    ranking induced by -KL(q||p) up to the entropy of q (constant per state).

    KL(p||q) = sum p log(p/q) strongly prefers uniform q when p is broad (typical
    CQT frames), so Viterbi collapsed to release/R states; weighting by q avoids
    that failure mode.
    """
    C_lin = np.power(10.0, C_db / 20.0)
    C_lin = np.maximum(C_lin, prob_floor)
    P = C_lin / (np.sum(C_lin, axis=0, keepdims=True) + 1e-12)

    Q = np.maximum(template_probs, prob_floor)
    Q /= np.sum(Q, axis=1, keepdims=True)
    logP = np.log(P + prob_floor)
    B = scale * (Q @ logP)
    return B, B
...
def emissions_ASR(
    ...
    B_base, scores = emissions_kl(C_db, template_probs, scale=scale_pitch)
    S, T = B_base.shape
...
def run(mp3_path: str, model_name: str, plot: bool = False):
    ...
    template_probs = build_ASR_template_prob_distributions(
        states,
        fmin=FMIN,
        n_bins=N_BINS,
        bins_per_octave=BINS_PER_OCTAVE,
