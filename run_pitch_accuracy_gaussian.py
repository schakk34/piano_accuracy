...
HMM emissions use Gaussian-weighted cosine correlation: sqrt(weight) * template
and sqrt(weight) * frame are L2-normalized, then dotted (scale * cosine). This
avoids weighted MSE collapsing when weights sit on quiet bins, which biased
Viterbi toward wrong (often lower) templates.
...
def note_template_linear(midi, fmin, n_bins, bins_per_octave, window_bins=2):
    tpl = np.zeros(n_bins, float)
    f0 = midi_to_hz(midi)
    harm_w = {1: 1.0, 2: 0.5, 3: 0.25}
    for h, w in harm_w.items():
...
def chord_template_linear(pitches, fmin, n_bins, bins_per_octave, window_bins=2):
    if not pitches:
        return np.ones(n_bins, dtype=float) * 1e-4
    tpl = np.zeros(n_bins, dtype=float)
    for midi in pitches:
        tpl += note_template_linear(midi, fmin, n_bins, bins_per_octave, window_bins=window_bins)
    return tpl


def gaussian_weight_vector_for_chord(
    pitches, fmin, n_bins, bins_per_octave, sigma_bins=1.5
):
    """
    Soft weights over frequency bins: sum of Gaussians centered on each
    expected partial, amplitude-scaled like the linear template harmonics.
    """
    if not pitches:
        return np.ones(n_bins, dtype=float)
    w = np.zeros(n_bins, dtype=float)
    harm_amp = {1: 1.0, 2: 0.5, 3: 0.25}
    b = np.arange(n_bins, dtype=float)
    for midi in pitches:
        f0 = midi_to_hz(midi)
        for h, amp in harm_amp.items():
            k = float(hz_to_cqt_bin(h * f0, fmin, bins_per_octave))
            k = np.clip(k, 0.0, float(n_bins - 1))
            w += amp * np.exp(-0.5 * ((b - k) / sigma_bins) ** 2)
...
def build_ASR_templates_gaussian_mse(
    states, fmin, n_bins, bins_per_octave, window_bins=2, sigma_bins=1.5
):
    """
    Linear templates + per-state Gaussian frequency weights for weighted MSE.
    Release uses uniform weights (all bins matter equally).
    """
    rows_t = []
    rows_w = []
    for st in states:
        ph = st["phase"]
        if ph in ("A", "S"):
            t = chord_template_linear(
                st["pitches"], fmin, n_bins, bins_per_octave, window_bins=window_bins
            )
            gw = gaussian_weight_vector_for_chord(
                st["pitches"], fmin, n_bins, bins_per_octave, sigma_bins=sigma_bins
            )
        else:
            t = np.ones(n_bins, dtype=float)
            gw = np.ones(n_bins, dtype=float)
        rows_t.append(t)
        rows_w.append(gw)
    return np.stack(rows_t, axis=0), np.stack(rows_w, axis=0)
...
def emissions_gaussian_mse(C_db, tpl_linear, tpl_weights, scale=1.0):
    """
    Frequency-weighted cosine in pseudo-amplitude space: x' = sqrt(w)*x,
    t' = sqrt(w)*template; emission = scale * dot(x'/||x'||, t'/||t'||).
    Same "higher is better" convention as emissions_cosine.
    """
    C_lin = np.power(10.0, C_db / 20.0)
    C_lin = np.maximum(C_lin, 1e-12)
    S = tpl_linear.shape[0]
    T = C_lin.shape[1]
    B = np.zeros((S, T), dtype=np.float64)
    w_stack = tpl_weights.astype(np.float64)
    sqrt_w = np.sqrt(np.maximum(w_stack, 1e-12))

    for s in range(S):
        sw = sqrt_w[s][:, np.newaxis]
        x = C_lin * sw
        tv = tpl_linear[s] * sqrt_w[s]
        tn = float(np.linalg.norm(tv)) + 1e-9
        t_unit = tv / tn
        xn = np.linalg.norm(x, axis=0, keepdims=True) + 1e-9
        x_unit = x / xn
        B[s, :] = scale * np.dot(t_unit, x_unit)
...
def emissions_ASR(
    ...
    B_base, scores = emissions_gaussian_mse(C_db, tpl_linear, tpl_weights, scale=scale_pitch)
    S, T = B_base.shape
...
def run(mp3_path: str, model_name: str, plot: bool = False):
    ...
    tpl_linear, tpl_weights = build_ASR_templates_gaussian_mse(
        states,
        fmin=FMIN,
        n_bins=N_BINS,
        bins_per_octave=BINS_PER_OCTAVE,
