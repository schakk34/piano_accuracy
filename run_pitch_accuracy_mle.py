...
HMM emissions use adaptive-threshold Bernoulli MLE on CQT bins:
- For each frame, compute threshold = mean(frame_energy) + k * std(frame_energy).
- Convert frame energies to a boolean active-bin vector.
- Convert each state template to an expected active-bin boolean vector.
- Score each (state, frame) by Bernoulli log-likelihood over bins.
...
def build_ASR_templates_binary(
    states, fmin, n_bins, bins_per_octave, window_bins=2, rel_thresh=0.12
):
    """
    Binary expected-activity templates for MLE emissions.
    A/S states: active bins where linear harmonic template is strong.
    R/START states: all False (silence-like expectation).
    """
    rows = []
    for st in states:
        if st["phase"] in ("A", "S"):
            t = np.zeros(n_bins, dtype=float)
            for midi in st["pitches"]:
                f0 = midi_to_hz(midi)
                harm_w = {1: 1.0, 2: 0.5, 3: 0.25}
                for h, w in harm_w.items():
                    k = hz_to_cqt_bin(h * f0, fmin, bins_per_octave)
            ...
            tmax = float(np.max(t))
            if tmax <= 1e-12:
                rows.append(np.zeros(n_bins, dtype=bool))
            else:
                rows.append(t >= (rel_thresh * tmax))
        else:
            rows.append(np.zeros(n_bins, dtype=bool))
...
def emissions_mle_threshold(
    C_db,
    template_active,
    scale=1.0,
    std_frac=0.85,
    p_active_given_expected=0.90,
    p_active_given_unexpected=0.08,
    outside_weight=0.2,
):
    """
    Adaptive-threshold Bernoulli MLE emission.

    C_db -> pseudo-amplitude C_lin.
    For each frame t:
      thr_t = mean(C_lin[:,t]) + std_frac * std(C_lin[:,t])
      x[:,t] = C_lin[:,t] >= thr_t (observed active bins)

    **Pitfall (harmony / wide chords):** Summing log-likelihood over *all* bins with
    the full (e_on, e_off) model makes note states pay ~O(n_bins) penalties on
    "expected on, observed off" while R only sees sparse x — R often wins and A/S
    coverage collapses → 0 notes detected.

    **Fix:** States with at least one expected-active bin score **only on those bins**
    (mean log-likelihood over template support), plus a **small** mean penalty on
    bins outside the template for spurious energy. R / silence states use the
    "prefer off everywhere" model averaged over all bins.
    """
    C_lin = np.power(10.0, C_db / 20.0)
    C_lin = np.maximum(C_lin, 1e-12)

    mu = np.mean(C_lin, axis=0, keepdims=True)
    sd = np.std(C_lin, axis=0, keepdims=True)
    thr = mu + std_frac * sd
    x = C_lin >= thr  # (n_bins, T)

    E = template_active.astype(bool)  # (S, n_bins)
    X = x.astype(np.float64)

    p1 = float(np.clip(p_active_given_expected, 1e-4, 1.0 - 1e-4))
    p0 = float(np.clip(p_active_given_unexpected, 1e-4, 1.0 - 1e-4))
    log_p1 = np.log(p1)
    log_1mp1 = np.log(1.0 - p1)
    log_p0 = np.log(p0)
    log_1mp0 = np.log(1.0 - p0)

    ll_on = X * log_p1 + (1.0 - X) * log_1mp1
    ll_off = X * log_p0 + (1.0 - X) * log_1mp0

    S = E.shape[0]
    T = X.shape[1]
    B = np.zeros((S, T), dtype=np.float64)
    n_bins = E.shape[1]
    ow = float(outside_weight)

    for s in range(S):
        e = E[s]
        n_expect = int(np.sum(e))
        if n_expect > 0:
            e_col = e[:, np.newaxis]
            num_on = np.sum(ll_on * e_col, axis=0)
            core = num_on / n_expect
            outside = np.sum(ll_off * (~e)[:, np.newaxis], axis=0) / max(n_bins - n_expect, 1)
            B[s, :] = scale * (core + ow * outside)
        else:
            B[s, :] = scale * np.mean(ll_off, axis=0)
...
def emissions_ASR(
    ...
    scale_pitch=2.0,
    mle_std_frac=0.5,
    mle_p_active_given_expected=0.90,
    mle_p_active_given_unexpected=0.08,
    mle_outside_weight=0.2,
    mle_note_prior=2.5,
    mle_start_penalty=1.25,
    w_attack_onset=2.0,
    w_sustain_onset=-0.5,
    w_release_onset=0.0,
    w_release_energy=-0.5,
):
    B_base, scores = emissions_mle_threshold(
        C_db,
        template_active,
        scale=scale_pitch,
        std_frac=mle_std_frac,
        p_active_given_expected=mle_p_active_given_expected,
        p_active_given_unexpected=mle_p_active_given_unexpected,
        outside_weight=mle_outside_weight,
    )
    S, T = B_base.shape

    # START uses the same "all bins quiet" emission as R; p_start_to_first is only 0.02 by default,
    # so Viterbi can stay in START forever (never enters seg 0 A/S → every segment "skipped").
    for s in range(S):
        if states[s]["phase"] == "START":
            B_base[s, :] -= mle_start_penalty
        elif np.any(template_active[s]):
            B_base[s, :] += mle_note_prior
...
class NoteDetectParams:
    ...
    # With harmony, segment max is often on the bass; melody can be >14 dB weaker and never becomes a
    # "strong" peak → false missing. Expected notes still pass if within this margin of segment max.
    expected_peak_margin_db: float = 26.0
    ...
    # Very short score segments (e.g. 0.5 beat) get few A∪S frames from Viterbi; widen the
    # slice used only for peak / energy analysis so we do not miss a real note at the boundary.
    short_segment_max_frames: int = 10
    eval_slice_pad_frames: int = 4
    short_slice_use_max_energy: int = 6      # if slice has fewer frames than this, use max not p90
...
def _compute_midi_energy_from_time(midi_energy_t, n_frames: Optional[int] = None, short_max: int = 6):
    """
    Convert MIDI->(per-frame) into MIDI->scalar energy using a robust percentile.
    For very few frames (short notes), p90 is unstable; use max instead.
    """
    out = {}
    for m, v in midi_energy_t.items():
        if v is None or len(v) == 0:
            continue
        if n_frames is not None and n_frames < short_max:
            out[m] = float(np.max(v))
        else:
            out[m] = float(np.percentile(v, 90))
    return out


def _analysis_frame_range(f0, f1, T, params: NoteDetectParams):
    """Widen [f0,f1] for peak detection when the HMM gave almost no A∪S frames."""
    span = f1 - f0 + 1
    if span > params.short_segment_max_frames:
        return f0, f1
    pad = params.eval_slice_pad_frames
    return max(0, f0 - pad), min(T - 1, f1 + pad)
...
def _candidate_peaks_for_segment(midi_energy, max_energy, exp_set, params: NoteDetectParams):
    """
    Union of global strong peaks and score-expected MIDI that are local maxima but weaker than harmony.
    """
    ...
    margin_exp = params.expected_peak_margin_db
    for p in exp_set:
        e = midi_energy.get(p)
        if e is None or e < max_energy - margin_exp:
            continue
        left = midi_energy.get(p - nb, -np.inf)
        right = midi_energy.get(p + nb, -np.inf)
        if e >= left and e >= right:
            peaks.add(p)
    return sorted(peaks)
...
def simple_segment_note_presence(
    ...
    for seg_idx in range(len(expected_segments)):
        ...
        if f1 <= f0:
            segment_max_energies.append(-np.inf)
            continue
        a0, a1 = _analysis_frame_range(f0, f1, T, params)
        slice_db = C_db[:, a0 : a1 + 1]
        midi_energy_t = _midi_energy_over_time(slice_db, fmin, bins_per_octave)
        nf = slice_db.shape[1]
        midi_energy = _compute_midi_energy_from_time(
            midi_energy_t, n_frames=nf, short_max=params.short_slice_use_max_energy
        )
    ...
    for seg_idx, (exp_pitches, _exp_dur_beats) in enumerate(expected_segments):
        ...
        # candidate peaks (harmony-tolerant: expected melody may be far below segment max)
...
def run(mp3_path: str, model_name: str, plot: bool = False):
    ...
    # tempo_slack > 1 lengthens expected phase dwell (smaller p_adv). For consecutive
    # same-pitch segments (e.g. final D4 pickup + D4 quarter), slack<1 made transitions
    # too eager and the short segment collapsed into the last one.
    states, A, pi = build_ASR_states_segments(
        segments, bpm_ref, dt_ms,
        frac_attack=0.15, frac_release=0.15,
        min_A=1, min_S=1, min_R=1,
        p_start_to_first=0.35,
        tempo_slack=1.06,
    )

    template_active = build_ASR_templates_binary(
        states,
        fmin=FMIN,
        n_bins=N_BINS,
        bins_per_octave=BINS_PER_OCTAVE,
    ...
    B, _ = emissions_ASR(
        C_db, template_active, onset_env, states,
        scale_pitch=2.0,
        mle_std_frac=0.65,
        mle_p_active_given_expected=0.90,
        mle_p_active_given_unexpected=0.08,
    ...
    params = NoteDetectParams(
        ...
        quiet_margin_db=8.0,
        presence_margin_db=18.0,
        expected_peak_margin_db=28.0,
        peak_neighbor=1,
        max_detune_semitones=0.45,
        match_window_semitones=2,
        octave_penalty_db=6.0,
        extra_presence_margin_db=10.0,
        extra_min_frames_ratio=0.30,
    )
